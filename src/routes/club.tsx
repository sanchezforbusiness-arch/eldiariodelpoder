    ...
    <section className="relative pt-28 pb-16 md:pt-44 md:pb-32 overflow-hidden grain">
      ...
      <div className="container-ddp relative grid md:grid-cols-12 gap-10 items-center">
        <div className="md:col-span-7 fade-up">
          ...
          <h1 className="text-2xl sm:text-display md:text-display lg:text-display leading-[0.95] sm:leading-[0.92] font-medium tracking-tight">
        ...
        <div className="grid md:grid-cols-3 gap-px bg-border border border-border reveal-stagger">
          {pillars.map((p) => (
            <div key={p.n} className="bg-background p-12 md:p-16 text-center">
              <p className="text-2xs tracking-label uppercase text-muted-foreground mb-4">{p.n}</p>
              <h2 className="tracking-tight text-2xl md:text-display italic text-foreground">{p.t}</h2>
            </div>
          ))}
        </div>
      </div>
 ...
 <section className="py-24 md:py-32 border-t border-border bg-card/20">
      <div className="container-ddp">
        ...
        <div className="grid md:grid-cols-2 gap-x-12 gap-y-10 reveal-stagger">
          {philosophy.map((p, i) => (
            <p key={i} className="font-serif text-2xl md:text-2xl lg:text-2xl leading-[1.15] font-light border-l-2 border-border pl-6">
              {p}
            </p>
          ))}
        </div>
            ...
            return (
              <div key={f.q} className="border-b border-border">
                <button onClick={() => setOpen(active ? null : i)} aria-expanded={active}
                  ...
                  <span className="font-serif text-xl md:text-2xl group-hover:text-signal transition-colors">{f.q}</span>
                  {active ? <Minus size={18} className="text-foreground shrink-0" /> : <Plus size={18} className="text-foreground shrink-0" />}
                </button>
